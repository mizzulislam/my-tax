...
export default function DashboardStats({ data }: { data: TaxReportData[] }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      <div className="group relative overflow-hidden rounded-3xl bg-slate-900/50 backdrop-blur-xl border border-slate-800 p-8 transition-all hover:bg-slate-800/50 hover:border-slate-700 hover:shadow-2xl hover:shadow-violet-500/10">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-12 h-12 rounded-2xl bg-violet-500/10 flex items-center justify-center text-violet-400 border border-violet-500/20 shadow-[0_0_15px_rgba(139,92,246,0.1)]">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 11h.01M12 7h.01M15 11h.01M15 7h.01M9 11h.01M4 19V5a2 2 0 012-2h12a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2z"></path></svg>
            </div>
            <span className="text-sm font-semibold text-violet-400/90 uppercase tracking-widest">Rasio Efektif</span>
          </div>
          <p className="text-3xl md:text-4xl font-bold text-white tracking-tight">
            {rawETR.toFixed(2)}<span className="text-slate-500 text-xl font-medium">% ETR</span>
          </p>
          <p className="text-sm text-slate-500 mt-4 leading-relaxed group-hover:text-slate-400 transition-colors">Rasio beban pajak kotor saat ini</p>
        </div>
      </div>

      {/* Card 5 - Total Penghasilan Gabungan */}
      <div className="group relative overflow-hidden rounded-3xl bg-slate-900/50 backdrop-blur-xl border border-slate-800 p-8 transition-all hover:bg-slate-800/50 hover:border-slate-700 hover:shadow-2xl hover:shadow-fuchsia-500/10">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-12 h-12 rounded-2xl bg-fuchsia-500/10 flex items-center justify-center text-fuchsia-400 border border-fuchsia-500/20 shadow-[0_0_15px_rgba(217,70,239,0.1)]">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            </div>
            <span className="text-sm font-semibold text-fuchsia-400/90 uppercase tracking-widest">Bruto Gabungan</span>
          </div>
          <p className="text-3xl md:text-4xl font-bold text-white tracking-tight">
            <span className="text-slate-500 text-2xl mr-1">Rp</span>
            {totalCombinedIncome.toLocaleString('id-ID')}
          </p>
          <p className="text-sm text-slate-500 mt-4 leading-relaxed group-hover:text-slate-400 transition-colors">Total penghasilan dari multi-sumber</p>
        </div>
      </div>

      {/* Card 6 - Total Kredit Pajak */}
      <div className="group relative overflow-hidden rounded-3xl bg-slate-900/50 backdrop-blur-xl border border-slate-800 p-8 transition-all hover:bg-slate-800/50 hover:border-slate-700 hover:shadow-2xl hover:shadow-cyan-500/10">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-12 h-12 rounded-2xl bg-cyan-500/10 flex items-center justify-center text-cyan-400 border border-cyan-500/20 shadow-[0_0_15px_rgba(6,182,212,0.1)]">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
            </div>
            <span className="text-sm font-semibold text-cyan-400/90 uppercase tracking-widest">Kredit Pajak</span>
          </div>
          <p className="text-3xl md:text-4xl font-bold text-white tracking-tight">
            <span className="text-slate-500 text-2xl mr-1">Rp</span>
            {totalWithheldTax.toLocaleString('id-ID')}
          </p>
          <p className="text-sm text-slate-500 mt-4 leading-relaxed group-hover:text-slate-400 transition-colors">Pajak dipotong pihak lain (Kredit Pajak)</p>
        </div>
      </div>
