...
export default function ChatQuiz({ content, isGenerating }: { content: string; isGenerating?: boolean }) {
  ...
  if (showSummary) {
    ...
    return (
      <div className="my-5 bg-gradient-to-br from-indigo-950/80 to-blue-950/80 border border-blue-500/35 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-250 text-center select-none">
        ...
        `}</style>
        <div className="w-16 h-16 bg-blue-500/10 border-2 border-blue-500/20 rounded-full flex items-center justify-center mx-auto mt-2 mb-4 text-3xl shadow-[0_0_25px_rgba(59,130,246,0.2)] animate-gentle-bounce">
          🏆
        </div>
      ...
      {!isSubmitted ? (
        ...
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-850 flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <button
            type="button"
            onClick={handleNext}
            className="px-3 sm:px-5 py-3 bg-blue-600 hover:bg-blue-500 font-extrabold text-white rounded-2xl transition-all text-[9px] sm:text-[10px] uppercase tracking-wider sm:tracking-widest cursor-pointer whitespace-normal sm:whitespace-nowrap self-stretch md:self-center flex items-center justify-center text-center shadow-lg leading-snug"
          >
