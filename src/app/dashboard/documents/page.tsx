...
const DocumentUploader = dynamic(() => import('@/components/documents/DocumentUploader'), {
  loading: () => (
    <div className="h-[360px] rounded-3xl border border-slate-800 bg-slate-900/40 animate-pulse" />
  ),
});

const DocumentCard = dynamic(() => import('@/components/documents/DocumentCard'), {
  loading: () => (
    <div className="h-48 rounded-2xl border border-slate-800 bg-slate-900/40 animate-pulse" />
  ),
});

const DocumentPreviewModal = dynamic(() => import('@/components/documents/DocumentPreviewModal'), {
  ssr: false,
});
